$(document).ready(function() {
			...
			success: function(result){
				...
				if(result.totalPages > 1 ){
			    	...
			    	$(".pageNumber").each(function(index){	
			    		if($(this).text() == page){ // là một câu lệnh điều kiện jQuery được sử dụng để kiểm tra nội dung của phần tử hiện tại có giống với page không.
	...
	$('#danhMucDropdown').mouseup(function() {
		...
		if (open) {
			...
			if ((label.toLowerCase()).indexOf("Laptop".toLowerCase()) != -1) { //m tra xem chuỗi label có chứa từ "laptop" hay không
	...
	// event khi ẩn modal form (nút duyệt sản phẩm)
	...
	//để xóa dữ liệu trong form sau khi đã thêm thành công sản phẩm.
	// btn Save Form Laptop Event ( nút xác nhận thêm laptop)
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			enctype: 'multipart/form-data',//Đây là kiểu mã hóa của dữ liệu gửi đi, trong trường hợp này là multipart/form-data để hỗ trợ gửi dữ liệu đa phần tử.
 			data : data,
 			
 		    // prevent jQuery from automatically transforming the data into a
			// query string
 	        processData: false,//jQuery sẽ ngăn không cho việc xử lý tự động của dữ liệu được gửi đi.
 	        contentType: false,// jQuery sẽ không đặt tiêu đề Content-Type trong yêu cầu HTTP.
 	        cache: false,
 	        timeout: 1000000,
